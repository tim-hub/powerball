...
Check `claude-code-dx.config.json` before execution:
...
Load configuration file:
  - claude-code-dx.config.json exists -> apply configuration
...
To execute the above plan, change the mode in claude-code-dx.config.json
```
